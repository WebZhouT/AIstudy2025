...
// src/core/utils/mouse-touch-event.ts
var MouseTouchEvent;
((MouseTouchEvent2) => {
  MouseTouchEvent2.isTouchEvent = (event) => "touches" in event;
  MouseTouchEvent2.touchToMouseEvent = (event) => {
    if (!(0, MouseTouchEvent2.isTouchEvent)(event)) {
      return event;
    }
    const touchEvent = event;
    const touch = touchEvent.touches[0] || touchEvent.changedTouches[0];
    if (touchEvent.type === "touchmove") {
      (0, MouseTouchEvent2.preventDefault)(touchEvent);
    }
  ...
  MouseTouchEvent2.getEventCoord = (e) => {
    if ((0, MouseTouchEvent2.isTouchEvent)(e)) {
      const touchEvent = e;
  ...
  MouseTouchEvent2.onTouched = (touchStartEvent, callback) => {
    ...
    const handleTouchEnd = (touchEndEvent) => {
      ...
      if (Math.abs(deltaX) < delta && Math.abs(deltaY) < delta) {
        const mouseEvent = (0, MouseTouchEvent2.touchToMouseEvent)(touchEndEvent);
        callback(mouseEvent);
      }
...
})(MouseTouchEvent || (MouseTouchEvent = {}));
...
var PlaygroundLayer = class extends Layer {
  ...
  onReady() {
    ...
    this.toDispose.pushAll([
      ...
      this.listenPlaygroundEvent("touchend", (e) => {
        this.options.hoverService?.clearHovered();
      }),
      this.listenPlaygroundEvent("touchcancel", (e) => {
        this.options.hoverService?.clearHovered();
      }),
      this.listenPlaygroundEvent(
        "mousedown",
...
import React4, { useMemo, useImperativeHandle, forwardRef, useRef, useEffect as useEffect2 } from "react";
...
var PlaygroundReactProvider = forwardRef(function PlaygroundReactProvider2(props, ref) {
  ...
  const effectSignalRef = useRef(0);
  useEffect2(() => {
...
import React8, { useEffect as useEffect4, useRef as useRef2 } from "react";
